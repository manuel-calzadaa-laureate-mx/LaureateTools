...
from files.object_addons_file import read_custom_data, ObjectAddonType, get_custom_indexes, GrantType
from files.table_column_substitute_file import get_tables_column_substitute_file
from tools.common_tools import MultiCounter, refactor_tagged_text, extract_object_structure, ObjectOriginType, \
    ObjectTargetType
...
def migrate_sequence_to_b9(b9_table_name: str,
    ...
    for custom_sequence in custom_sequences:
        ...
        drop_synonyms = read_custom_data(b9_object_name=sequence_name, object_addon_type=ObjectAddonType.DROP_SYNONYMS,
                                         b9_object_owner="UVM")
        new_sequence = {
...
def migrate_b9_table_to_b9(json_data: dict, b9_table_name: str,
    ...
    revokes = read_custom_data(b9_object_name=b9_table_name, object_addon_type=ObjectAddonType.REVOKES,
                               grant_type=GrantType.TABLE,
                               b9_object_owner="UVM")
    synonyms = read_custom_data(b9_object_name=b9_table_name, object_addon_type=ObjectAddonType.SYNONYMS,
                                b9_object_owner="UVM")

    drop_synonyms = read_custom_data(b9_object_name=b9_table_name, object_addon_type=ObjectAddonType.DROP_SYNONYMS,
                                     b9_object_owner="UVM")
